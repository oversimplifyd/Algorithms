  ...
  public static int findMinimumMeetingRooms(List<Meeting> meetings) {
    ...
    while (i < meetings.size()) {
      if (meetings.get(i).start < meetings.get(i - 1).end) {
        ...
        if (!(meetings.get(i).start >= earliestClosingTime && earliestClosingTime > 0)) {
          roomCount++; 
        } 
        earliestClosingTime = Math.min(meetings.get(i).end, meetings.get(i-1).end); 
      } else {
        maxRooms = Math.max(maxRooms, roomCount);
        roomCount = 1; 
        earliestClosingTime = 0; 
      }

      i++; 
    }

    return Math.max(maxRooms, roomCount); 
  }
